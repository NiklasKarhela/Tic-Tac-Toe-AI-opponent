...
def player_move(rounds):
    ...
    if choise in positions:  #tittar om inputten är i rätt form (t.ex A1)
        if check_taken(choise) == 0:  #tittar om valda rutan är ledig
            ...
            print("Rutan är tagen! ")
            player_move(rounds)
    else:
        print("Fel inmatning (A1, A2, A3, B1, B2, B3, C1, C2 och C3 godkäns)")
        player_move(rounds)
...
def bot_move(rounds):
    ...
    for x in range(3):
        for z in range(3):
            if matrix[x][z] == "": #Tittar om rutan är ledig (forloopina går genom alla positioner)
